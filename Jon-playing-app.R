...
ui <- fluidPage(
  titlePanel("Medical School Financial Planning"),
    sidebarLayout(
      ...
      mainPanel(
        tabsetPanel(
          tabPanel("Other Salaries", plotOutput("newsals")),
...
output$newsals <- renderPlot({
  othersalaries_standard <- vector(length=20)
  othersalaries_ten <- vector(length=20)
  othersalaries_twenty <- vector(length=20)
  othersalaries_underserved <- vector(length=20)
  othersalaries_military <- vector(length=20)
  testsal <- vector(length=20)
...
for(bigfor in 1:20) {
  ...
  if(res<=5) {for(i in 1:(res+4)) {military.gross[i] <- in.military[i]}#military residency+service years salary
    for(j in (res+5):20) {military.gross[j] <- 50000*bigfor} }#civilian physician salary
  if(res>5) {for(i in 1:(res+(res-1))) {military.gross[i] <- in.military[i]}#military residency+service years salary
    for(j in (res+res):20) {military.gross[j] <- 50000*bigfor}} #civilian physician salary
  years <- c(1:20)
  ...
  othersalaries_standard[bigfor] <- cum_disposable_standard[20]
  othersalaries_ten[bigfor] <- cum_disposable_ten[20]
  othersalaries_twenty[bigfor] <- cum_disposable_twenty[20]
  othersalaries_underserved[bigfor] <- cum_disposable_underserved[20]
  othersalaries_military[bigfor] <- cum_disposable_military[20]
  testsal[bigfor] <- salary
}
  
All.other.sal <- data.frame(testsal, othersalaries_standard, othersalaries_ten, othersalaries_twenty, othersalaries_underserved, othersalaries_military)
  
  
  
  p1 <- ggplot(All.other.sal, aes(x = testsal))
